...
export default function YouTubeEmbed(props: Props) {
  ...
  const src =
    "videoId" in props
      ? `https://www.youtube-nocookie.com/embed/${encodeURIComponent(props.videoId)}?autoplay=1&playsinline=1&rel=0&modestbranding=1&controls=1${
          muted ? "&mute=1" : ""
        }`
      : `https://www.youtube-nocookie.com/embed/live_stream?channel=${encodeURIComponent(
          props.channelId
        )}&autoplay=1&playsinline=1&rel=0&modestbranding=1&controls=1${muted ? "&mute=1" : ""}`;
