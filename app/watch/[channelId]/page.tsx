...
/** Robust duration + “is live” helpers (UTC) */
const DEFAULT_SECONDS = 1800; // 30m when duration is null/0/weird
...
function isLiveUTC(p: { start_time: string; duration: number }, nowMs: number) {
  const start = new Date(p.start_time).getTime(); // parses ISO UTC fine
  ...
  const fetchCurrentProgram = useCallback(
    async (numericChannelId: number) => {
      ...
      try {
        ...
        // Get items around "now" using ONLY your real columns
        const { data: startedRows, error: e1 } = await supabase
          ...
          .limit(12);
        if (e1) throw new Error(`Database error: ${e1.message}`);
        ...
        const { data: upcomingRows, error: e2 } = await supabase
          ...
          .limit(6);
        if (e2) throw new Error(`Database error: ${e2.message}`);
        ...
        // choose: live → next upcoming → most recent past; prefer one that has mp4_url
        let candidate =
          started.find(p => isLiveUTC(p as any, nowMs)) ||
          (upcoming.length ? upcoming[0] : undefined) ||
          (started.length ? started[0] : undefined) ||
          null;
        ...
        const nextProgram = candidate ?? getStandbyMp4Program(numericChannelId, new Date());
        ...
        const nextPoster =
          (nextProgram as any)?.poster_url || (channelDetails as any)?.image_url || undefined;
