...
export function VideoPlayer({ channel, initialProgram, upcomingPrograms: initialUpcoming }: VideoPlayerProps) {
  ...
  const getVideoType = (url: string): "mp4" | "hls" | "unknown" => {
    ...
    if (lowerUrl.includes("mux.com") || lowerUrl.includes("livestream") || lowerUrl.includes("stream")) {
      return "hls"
    }
  ...
  const loadVideo = (url: string, forceRetry = false) => {
    ...
    addDebugInfo("useCorsProxy", useCorsProxy)
    ...
    if (useCorsProxy) {
      fullUrl = applyCorsProxy(fullUrl)
      console.log("Applied CORS proxy, new URL:", fullUrl)
      addDebugInfo("proxiedUrl", fullUrl)
    }
    ...
    const timeout = setTimeout(() => {
      ...
      if (isLoading) {
        ...
        if (retryCount < maxRetries) {
          ...
          if (retryCount === 0) {
            // First retry: Try with cache busting
            loadVideo(url, false)
          } else if (retryCount === 1) {
            // Second retry: Try with CORS proxy
            setUseCorsProxy(true)
            loadVideo(url, false)
          } else {
            // Third retry: Try with original URL again
  ...
  const handleVideoError = (e: React.SyntheticEvent<HTMLVideoElement>) => {
    ...
    if (retryCount < maxRetries) {
      ...
      if (currentProgram?.mp4_url) {
        setTimeout(() => {
          // On first retry, try adding cache-busting parameter
          if (retryCount === 0) {
            const cacheBuster = `?cb=${Date.now()}`
            const urlWithCache = currentProgram.mp4_url.includes("?")
              ? `${currentProgram.mp4_url}&cb=${Date.now()}`
              : `${currentProgram.mp4_url}${cacheBuster}`
            loadVideo(urlWithCache, false)
          }
          // On second retry, try with CORS proxy
          else if (retryCount === 1) {
            setUseCorsProxy(true)
            loadVideo(currentProgram.mp4_url, false)
          }
          // On third retry, try with a different content type
          else {
            // Force the opposite video type
            const currentType = videoType
            setVideoType(currentType === "mp4" ? "hls" : "mp4")
  ...
  const toggleCorsProxy = () => {
    const newValue = !useCorsProxy
    setUseCorsProxy(newValue)
    console.log(`CORS proxy ${newValue ? "enabled" : "disabled"}`)

    // Reload the video with the new setting
    if (currentProgram?.mp4_url) {
      loadVideo(currentProgram.mp4_url, true)
    }
  }
    ...
    <div className="relative bg-black">
          ...
          <div className="flex flex-col items-center">
            ...
            {retryCount > 0 && (
              <p className="text-gray-400 text-sm mt-2">
                {useCorsProxy ? "Trying with CORS proxy..." : "If loading fails, try refreshing the page."}
              </p>
            )}
          </div>
        </div>
            ...
            <div className="mt-4">
              <button
                ...
                className={`px-4 py-2 rounded transition-colors ${
                  useCorsProxy ? "bg-green-600 hover:bg-green-700" : "bg-blue-600 hover:bg-blue-700"
                } text-white`}
              >
                {useCorsProxy ? "Disable CORS Proxy" : "Try with CORS Proxy"}
              </button>
              <p className="text-gray-400 text-xs mt-2">
                {useCorsProxy
                  ? "CORS proxy is enabled. This may help with cross-origin issues."
                  : "If you're experiencing CORS issues, try enabling the proxy."}
              </p>
            </div>
          </div>
        </div>
      ...
      <div ref={videoContainerRef} className="w-full aspect-video bg-black">
        {videoUrl ? (
          <video
            ref={videoRef}
            key={`${videoUrl}-${retryCount}-${useCorsProxy ? "proxy" : "direct"}`} // This forces a complete remount when the URL, retry count, or proxy setting changes
      ...
      <div className="bg-black p-2 text-xs text-gray-500">
        ...
        <p>CORS Proxy: {useCorsProxy ? "Enabled" : "Disabled"}</p>
