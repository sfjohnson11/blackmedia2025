...
export function ChannelCard({ channel, userTier = null }: ChannelCardProps) {
  const idNum = typeof channel.id === 'string' ? Number.parseInt(channel.id, 10) : (channel.id as number)
  const cleanedName = cleanChannelName(channel.name ?? `Channel ${idNum}`)
  const imageUrl = channel.logo_url || `https://placehold.co/400x225?text=${encodeURIComponent(cleanedName)}`
  ...
  const href = needsMembership && !isUnlocked ? '/membership' : `/watch/${channel.id}`

  return (
    <Link
      href={href}
      className="block"
      aria-label={`Open ${cleanedName}${needsMembership && !isUnlocked ? ' (membership required)' : ''}`}
    >
